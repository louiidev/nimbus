...
use crate::{
    material::Material,
    mesh::{AttributeValue, Indices, MeshBuilder},
};
...
impl From<Box> for Mesh {
    fn from(sp: Box) -> Self {
        ...
        let positions: Vec<_> = vertices
            .iter()
            .map(|(p, _, _)| AttributeValue::Position(*p))
            .collect();
        ...
        let mesh_builder = MeshBuilder::new(Material::default())
            .with_attributes(crate::mesh::MeshAttribute::Position, positions)
            .with_attributes(crate::mesh::MeshAttribute::Normal, normals)
