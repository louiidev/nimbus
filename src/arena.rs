// Source: https://github.com/ChevyRay/arena
...
impl<T> Arena<T> {
    ...
    ///
    /// ```
    /// # use crate::renderer::arena::{Arena, ArenaId};
    /// struct Person {
    ///     id: Arena<T>,
    ///     name: &'static str,
    /// }
...
pub struct WeakArenaId {
    uid: u64,
    idx: usize,
}

impl<T> From<WeakArenaId> for ArenaId<T> {
    fn from(value: WeakArenaId) -> Self {
        ArenaId {
            uid: value.uid,
            idx: value.idx,
            marker: PhantomData,
...
impl<T> From<ArenaId<T>> for WeakArenaId {
...
impl<T> PartialEq for ArenaId<T> {
    fn eq(&self, other: &Self) -> bool {
        self.idx == other.idx && self.uid == other.uid
    }
}
...
// `(float_ord::FloatOrd, ArenaId<FontAtlas>): std::cmp::Eq`
// `(float_ord::FloatOrd, ArenaId<FontAtlas>): Hash`
